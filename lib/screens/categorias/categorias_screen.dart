...
class _ModuloCategoriasScreenState extends State<ModuloCategoriasScreen> {
  ...
  Future<void> _cargarCategoriasCombinadas() async {
    try {
      ...
      final combinadas = gastos.map((cat) {
        final ing = ingresos.firstWhere(
          (c) => c['id'] == cat['id'],
          orElse: () => <String, dynamic>{},
        );
        final aho = ahorros.firstWhere(
          (c) => c['id'] == cat['id'],
          orElse: () => <String, dynamic>{},
        );
        final imp = imprevistos.firstWhere(
          (c) => c['id'] == cat['id'],
          orElse: () => <String, dynamic>{},
        );
        final deu = deudas.firstWhere(
          (c) => c['id'] == cat['id'],
          orElse: () => <String, dynamic>{},
        );
  ...
  List<CategoriaModel> get _activas =>
      _categorias.where((c) => c.activa).toList();

  List<CategoriaModel> get _inactivas =>
      _categorias.where((c) => !c.activa).toList();
      ...
      if (respuesta['ok'] == true) {
        ...
        setState(() {
          _categorias = _categorias
              .map((c) => c.id == id ? c.copyWith(activa: false) : c)
              .toList();
        });
      } else {
        await _mostrarAlerta(
          respuesta['mensaje'] as String? ?? 'Error al deshabilitar la categoría',
        );
      }
    } catch (error) {
      await _mostrarAlerta('Error al deshabilitar la categoría: $error');
    }
  }
  ...
  Future<void> _handleHabilitar(int id) async {
    try {
      ...
      if (respuesta['ok'] == true) {
        ...
        setState(() {
          _categorias = _categorias
              .map((c) => c.id == id ? c.copyWith(activa: true) : c)
              .toList();
        });
      } else {
        await _mostrarAlerta(
          respuesta['mensaje'] as String? ?? 'Error al habilitar la categoría',
        );
      }
    } catch (error) {
      await _mostrarAlerta('Error al habilitar la categoría: $error');
    }
  }
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const Center(
              child: Text(
                '© 2026 Ahorrapp. Todos los derechos reservados.',
                style: TextStyle(fontSize: 11, color: _zinc600),
              ),
            ),
          ],
        ),
            ...
            child: Column(
              ...
              children: [
                ...
                if (_inactivas.isNotEmpty) ...[
                  ...
                  Text(
                    'Categorías deshabilitadas (${_inactivas.length})',
                    style: const TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.w700,
                      color: _zinc500,
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Text(
                            cat.descripcion?.isNotEmpty == true
                                ? cat.descripcion!
                                : 'Sin descripción',
                            style: const TextStyle(
                              fontSize: 13,
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                _formatMoney(cat.totalMovimientos),
                                style: const TextStyle(
                                  fontSize: 12,
                                  fontWeight: FontWeight.w700,
                                  color: _amber300,
              ...
              // 👇 Botones solo para categorías creadas por el usuario
              if (!cat.esSistemaOGlobal) ...[
                const SizedBox(height: 14),
                Row(
                  children: [
                    Expanded(
                      child: _botonEditar(() => _abrirEditarCategoria(cat)),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _botonDeshabilitar(
                        () => _handleDeshabilitar(cat.id),
                      ),
                    ),
                  ],
                ),
      ...
      child: DataTable(
        columns: const [
          DataColumn(
            label: Text(
              'Nombre',
              style: TextStyle(color: _zinc500, fontWeight: FontWeight.w700),
            ),
          ),
          DataColumn(
            label: Text(
        ...
        rows: _activas.map((cat) {
          return DataRow(
            cells: [
              ...
              DataCell(
                Text(
                  cat.descripcion?.isNotEmpty == true ? cat.descripcion! : '—',
                  style: const TextStyle(color: _zinc400),
                ),
              ),
            ...
            Padding(
              ...
              child: Text(
                cat.descripcion?.isNotEmpty == true
                    ? cat.descripcion!
                    : 'Sin descripción',
                style: const TextStyle(fontSize: 13, color: _zinc600),
              ),
            ),
            const SizedBox(height: 12),
            SizedBox(
              ...
              child: OutlinedButton(
                ...
                style: OutlinedButton.styleFrom(
                  foregroundColor: _emerald400,
                  side: const BorderSide(color: _emerald400),
                ),
  ...
  Widget _chipTipo(bool esGlobal) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: (esGlobal ? _emerald400 : _indigo400).withOpacity(0.4),
        ),
        borderRadius: BorderRadius.circular(20),
      ),
      child: Text(
        esGlobal ? 'Sistema' : 'Personal',
        style: TextStyle(
          fontSize: 11,
          fontWeight: FontWeight.w700,
          color: esGlobal ? _emerald400 : _indigo300,
        ),
      ),
    );
  }
  ...
  Widget _botonDeshabilitar(VoidCallback onTap) {
    return OutlinedButton(
      onPressed: onTap,
      style: OutlinedButton.styleFrom(
        foregroundColor: _orange400,
        side: const BorderSide(color: _orange400),
      ),
      child: const Text(
        'Deshabilitar',
        style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700),
