...
class _AgregarDeudaScreenState extends State<AgregarDeudaScreen> {
  ...
  void initState() {
    ...
    if (widget.deudaParaEditar != null) {
      ...
      _montoController.text = (d.monto % 1 == 0) ? d.monto.toStringAsFixed(0) : d.monto.toString();
  ...
  CategoriaModel? get _categoriaSeleccionadaModel {
    ...
    try { return _categorias.firstWhere((c) => c.id == _categoriaSeleccionada); } catch (_) { return null; }
  }
  ...
  void _guardar() async {
    ...
    try {
      if (deuda.id == null) { await DeudasService.crearDeuda(deuda); } else { await DeudasService.actualizarDeuda(deuda.id!, deuda); }
      if (!mounted) return;
  ...
  Widget _buildHeader() {
    return NeumorphicHeader(
      title: widget.deudaParaEditar != null ? 'Editar Deuda' : 'Nueva Deuda',
      subtitle: 'Registrar obligación financiera',
  ...
  Widget _buildFormCard() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const NeumorphicLabel(text: 'Acreedor / Fuente', required: true),
        const SizedBox(height: 8),
        NeumorphicTextField(
          controller: _fuenteController,
          hint: 'Ej: Banco, Juan Pérez',
          icon: Icons.badge_outlined,
          iconColor: AppModuleColors.deudas,
          validator: (v) => v!.isEmpty ? 'Campo requerido' : null,
        ),
        const SizedBox(height: 18),
        const NeumorphicLabel(text: 'Monto Total', required: true),
        const SizedBox(height: 8),
        NeumorphicTextField(
        ...
        const SizedBox(height: 18),
        const NeumorphicLabel(text: 'Número de Cuotas'),
        const SizedBox(height: 8),
        NeumorphicTextField(
        ...
        const SizedBox(height: 18),
        const NeumorphicLabel(text: 'Fecha Estimada de Fin'),
        const SizedBox(height: 8),
        _buildFechaFinField(),
      ],
    );
  ...
  Widget _buildCategoriaField() {
    ...
    return NeumorphicActionField(
      onTap: _isLoadingCategorias ? () {} : _showCategorySheet,
      icon: cat != null ? _getIconForCategory(cat.nombre) : Icons.category_outlined,
      iconColor: cat != null ? _getColorForCategory(cat.nombre) : AppModuleColors.deudas,
      value: cat?.nombre,
      hint: _isLoadingCategorias ? 'Cargando...' : 'Sin categoría',
    );
  }
  ...
  Widget _buildFechaFinField() {
    return NeumorphicActionField(
      ...
      iconColor: AppModuleColors.deudas,
      value: _fechaFin == null ? null : _formatFecha(_fechaFin!),
      hint: 'Sin definir',
    );
  }
          ...
          children: [
            ...
            GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final isSelected = _fechaFin != null && day == _fechaFin!.day && month == _fechaFin!.month && year == _fechaFin!.year;
                return GestureDetector(
                  onTap: () { setState(() => _fechaFin = dayDate); Navigator.pop(context); },
                  child: Container(
                    margin: const EdgeInsets.all(3),
                    decoration: isSelected ? const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [AppModuleColors.deudas, Color(0xFFFF8A8A)])) : const BoxDecoration(color: AppColors.background, shape: BoxShape.circle),
            ...
            if (_fechaFin != null) ...[
              const SizedBox(height: 12),
              Center(child: TextButton(onPressed: () { setState(() => _fechaFin = null); Navigator.pop(context); }, child: const Text('Quitar fecha', style: TextStyle(color: AppColors.textSecondary, fontWeight: FontWeight.w600)))),
            ],
          ],
        ),
      ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isSelected ? Border.all(color: AppModuleColors.deudas.withValues(alpha: 0.6), width: 1.5) : null,
  ...
  Widget _buildGuardarButton() {
    return NeumorphicPrimaryButton(
      label: widget.deudaParaEditar != null ? 'Guardar cambios' : 'Guardar Deuda',
      onTap: _guardar,
  ...
  Color _getColorForCategory(String nombre) {
    switch (nombre) {
      ...
      default: return AppModuleColors.deudas;
    }
  }
}
      ...
      builder: (context, child) {
        return Transform.translate(
          ...
          child: Container(
            ...
            child: const Icon(Icons.keyboard_arrow_down, color: AppModuleColors.deudas, size: 24),
          ),
        );
      },
    );
